...
class multi_net(object):
	...
	def train(self, cnn, trX, trY, teX, teY):
		...
		for start, end in zip(range(0, len(teX), self.batch_size), range(self.batch_size, len(teX), self.batch_size)):
			accuracy = self.calculate_accuracy(teX[start:end], teY[start:end])
			accuracies.append(accuracy)
		print("task_id: {0} accuracy: {1}".format(self.task_id, np.mean(accuracies)))
	...
	def calculate_accuracy(self,teX, teY):
		...
		for i in range(len(teX)):
			index = np.argmax(teY[i])
			if index == 0:
				if len(self.net_list) >= 1:
					cnn = self.net_list[index]
				else:
					cnn = self.cnn
			else:
				if index <= len(self.net_list):
					cnn = self.net_list[index-1]
				else:
					cnn = self.cnn
