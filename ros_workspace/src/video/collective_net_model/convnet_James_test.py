...
class ConvolutionalNeuralNetwork(object):
	...
	def model(self, X, w1, w2, w3, w4, wo, p_drop_conv, p_drop_hidden):
		...
		signal = T.dot(l4, wo)
		pyx = self.softmax(signal)
		return l1, l2, l3, l4, pyx, signal
	...
	def create_model_functions(self):
		self.noise_l1, self.noise_l2, self.noise_l3, self.noise_l4, self.noise_py_x, _ = self.model(self.X, self.w1, self.w2, self.w3, self.w4, self.wo, 0.2, 0.5)
		self.l1, self.l2, self.l3, self.l4, self.py_x, self.signal = self.model(self.X, self.w1, self.w2, self.w3, self.w4, self.wo, 0., 0.)
		...
		self.binarize = theano.function(inputs = [self.X], outputs = self.signal, allow_input_downcast = True)
	...
	def save_all_weights(self, task_id):
		self.save_data("saved/{0}/W1.txt".format(task_id), self.w1, gpu = True)
		self.save_data("saved/{0}/W2.txt".format(task_id), self.w2, gpu = True)
		self.save_data("saved/{0}/W3.txt".format(task_id), self.w3, gpu = True)
		self.save_data("saved/{0}/W4.txt".format(task_id), self.w4, gpu = True)
		self.save_data("saved/{0}/Wo.txt".format(task_id), self.wo, gpu = True)
