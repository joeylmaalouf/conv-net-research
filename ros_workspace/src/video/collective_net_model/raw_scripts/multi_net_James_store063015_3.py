...
class multi_net(object):
	...
	def train(self, cnn, trX, trY, teX, teY):
		for i in range(self.epochs):
			...
			self.calculate_accuracy(cnn, "cnn", teX, teY)
			if self.task_id-3 >= 0:
				self.calculate_accuracy(self.net_list[self.task_id-3], "cnn_3", teX, teY)
			if self.task_id-6 >= 0:
				self.calculate_accuracy(self.net_list[self.task_id-6], "cnn_6", teX, teY)


	def calculate_accuracy(self, cnn, flag, teX, teY):
#		accuracies = []
		predict = np.argmax(self.cnn.predict(teX), axis = 1)
