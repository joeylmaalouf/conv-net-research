...
class ConvolutionalNeuralNetwork(object):
	...
	def create_model_functions(self):
		...
		self.activate4 = theano.function(inputs = [self.X], outputs = self.l4, allow_input_downcast = True)
		self.binarize = theano.function(inputs = [self.X], outputs = self.sig, allow_input_downcast = True)
		self.activate3 = theano.function(inputs = [self.X], outputs = self.l3, allow_input_downcast = True)
		self.activate2 = theano.function(inputs = [self.X], outputs = self.l2, allow_input_downcast = True)
		self.activate1 = theano.function(inputs = [self.X], outputs = self.l1, allow_input_downcast = True)
	...
	def save_all_weights(self):
		self.save_data("saved/l1.txt", self.l1, gpu = True)
		self.save_data("saved/l2.txt", self.l2, gpu = True)
		self.save_data("saved/l3.txt", self.l3, gpu = True)
		self.save_data("saved/l4.txt", self.l4, gpu = True)
		self.save_data("saved/pyx.txt", self.pyx, gpu = True)

	def load_all_weights(self, task_id):
		self.w1 = self.load_data("saved/{0}/W1.txt".format(task_id), (32, 1, 3, 3), gpu = True)
		self.w2 = self.load_data("saved/{0}/W2.txt".format(task_id), (64, 32, 3, 3), gpu = True)
		self.w3 = self.load_data("saved/{0}/W3.txt".format(task_id), (128, 64, 3, 3), gpu = True)
		self.w4 = self.load_data("saved/{0}/W4.txt".format(task_id), (128 * 3 * 3, 625), gpu = True)
		self.wo = self.load_data("saved/{0}/Wo.txt".format(task_id), (625, 10), gpu = True)

	def mnist_example(self, verbose = False, save = False):
		self.initialize_mnist()
		self.create_model_functions()
		self.train_mnist(verbose, epochs = 1)
		if save:
			# self.save_all_weights()
			# print("Saved weights to \"./saved/W*.txt\".")
			num_chunks = 40
			for i in range(num_chunks):
				data_chunk = self.trX[(len(self.trX[:100])/num_chunks*i):(len(self.trX[:100])/num_chunks*(i+1))]
				# print self.activate4(data_chunk).shape
				# print self.activate3(data_chunk).shape
				# print self.activate2(data_chunk).shape
				# print self.activate1(data_chunk).shape
				self.save_data("saved/act4{0:02d}.txt".format(i), self.predict(data_chunk))
				self.save_data("saved/act3{0:02d}.txt".format(i), self.activate3(data_chunk))
				self.save_data("saved/act2{0:02d}.txt".format(i), self.activate2(data_chunk))
				self.save_data("saved/act1{0:02d}.txt".format(i), self.activate1(data_chunk))
			# self.save_data("saved/teA.txt", self.activate4(self.teX))
