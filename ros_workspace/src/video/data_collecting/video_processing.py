		...
		while True:
			...
			if self.saving and self.new_frame:
				...
				if len(self.refPt) == 2:
						...
						cv2.imwrite('frames/full{0}({1}).png'.format(self.category, self.time),self.frame)
						cv2.imwrite('frames/{0}({1}) .png'.format(self.category,self.time),roi)
						image_file = open("frames/{0}({1}).txt".format(self.category, self.time), "w")
