...
class ConvolutionalNeuralNetwork(object):
	...
	def save_all_weights(self, task_id):
		self.save_data("saved/{0}/W1.txt".format(task_id), self.w1, gpu = False)
		self.save_data("saved/{0}/W2.txt".format(task_id), self.w2, gpu = False)
		self.save_data("saved/{0}/W3.txt".format(task_id), self.w3, gpu = False)
		self.save_data("saved/{0}/W4.txt".format(task_id), self.w4, gpu = False)
		self.save_data("saved/{0}/Wo.txt".format(task_id), self.wo, gpu = False)

	def load_all_weights(self, task_id):
		self.w1 = self.load_data("saved/{0}/W1.txt".format(task_id), (32, 1, 3, 3), gpu = False)
		self.w2 = self.load_data("saved/{0}/W2.txt".format(task_id), (64, 32, 3, 3), gpu = False)
		self.w3 = self.load_data("saved/{0}/W3.txt".format(task_id), (128, 64, 3, 3), gpu = False)
		self.w4 = self.load_data("saved/{0}/W4.txt".format(task_id), (128 * 3 * 3, 625), gpu = False)
		self.wo = self.load_data("saved/{0}/Wo.txt".format(task_id), (625, 10), gpu = False)
